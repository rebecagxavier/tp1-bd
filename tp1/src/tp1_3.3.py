...
def consulta1(asin, cursor):
        ...
        query1 = f"SELECT id_avaliacao FROM avaliacoes_produto WHERE asin = '{asin}'"
        cursor.execute(query1)
        id_ava = cursor.fetchall()[0][0]
        query2 = f"SELECT id_cliente, nota, votos, util, data FROM avaliacao WHERE id_avaliacao = {id_ava} ORDER BY util DESC, nota DESC LIMIT 5"
        cursor.execute(query2)
        lista_5_u_maior = cursor.fetchall()
        query3 = f"SELECT id_cliente, nota, votos, util, data FROM avaliacao WHERE id_avaliacao = {id_ava} ORDER BY util DESC, nota ASC LIMIT 5"
...
def consulta3(asin, cursor):
        ...
        query1  = f'''
                ...
                FROM avaliacao 
                WHERE id_avaliacao = 
                        (SELECT avaliacoes_produto.id_avaliacao
...
def consulta5(cursor):
        query1 = '''
                SELECT id_avaliacao, ROUND(avg(util),2) as media_util
                FROM avaliacao
...
def consulta6(cursor):
	...
	categoria JOIN
        ...
        ON categoria.id_produto = avaliacao_medias.id_avaliacao
...
def consulta7(cursor):
        ...
        query1 = '''
                WITH ranking AS (
                        ...
                        FROM avaliacao
                        JOIN avaliacoes_produto 
                                ON avaliacao.id_avaliacao = avaliacoes_produto.id_avaliacao
                        JOIN produto 
                                ON produto.asin = avaliacoes_produto.asin
