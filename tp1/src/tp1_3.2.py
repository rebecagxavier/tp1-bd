...
def povoa(cursor, arquivo_entrada):
    ...
    query_avaliacao = "INSERT INTO avaliacao (id_avaliacao, asin, id_cliente, nota, votos, util, data)\nVALUES\n"
    avaliacao_sql = []
    ...
    for k, produto in enumerate(produtos):
        ...
        avaliacao_produto_sql.append(f"('{asin}', {coms_total}, {coms_baixados}, {avg})")
        ...
        if comentarios is not None:
            for comentario in comentarios:
                ...
                avaliacao_sql.append (f"('{asin}', '{id_cliente}', {nota}, {votos}, {util}, '{data}')")
        
        #insercao categorias
        for categoria in categorias:
            categoria_sql.append(f"('{asin}', '{categoria}')")
        print(k)
